...
const string PerformerAttention::LAYER_NAME = "PerformerAttention";

PerformerAttention::PerformerAttention(int numHeads, int keySize, int valueSize, int M = 50) {
...
void PerformerAttention::propagateLayer(int num) {
...
void PerformerAttention::backPropagate(int num) {
...
void PerformerAttention::setPrevLayer(Layer* prevLayer) {
...
void PerformerAttention::setBatchSize(int batchSize) {
...
void PerformerAttention::save(ofstream& file) {
	file << LAYER_NAME << ",";
...
void PerformerAttention::load(Model* nn, ifstream& file, string& line, int* commaIndex, int* newCommaIndex, int* prevSize) {
	...
	PerformerAttention* multiHeadAttentionLayer = { new PerformerAttention(numHeads, keySize, valueSize) };
...
void PerformerAttention::applyGradients(float learningRate, int t) {
	for (int i = 0; i < batchSize; i++) {
...
void PerformerAttention::setOptimizer(Optimizer* optimizer) {
...
int PerformerAttention::getNumParameters() {
...
void PerformerAttention::phi(int height, int width, Matrix X, Matrix Y) {
	Matrix::multiplyABC(M, height, width, omega, X, Y, true);
}